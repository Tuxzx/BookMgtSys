...
public class UserDaoImpl implements UserDao {
    ...
    private static final String QUERY_MANAGER_INFO = "select * from student_info where manager_id = ? and manager_password = ?;";
    ...
    public Manager getManager(String managerId) {
        return jdbcTemplate.queryForObject(QUERY_MANAGER_INFO, new ManagerMapper());
    }
    ...
    public boolean resetPassword(String id, String password, boolean isAdmin) {
        if (isAdmin)
            return jdbcTemplate.update(UPDATE_MANAGER_PASSWORD,new Object[]{id, password})>0?true:false;
        else
            return jdbcTemplate.update(UPDATE_USER_PASSWORD,new Object[]{id, password})>0?true:false;
    }
